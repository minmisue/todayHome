...
public class FollowController {
	...
	public String followingForm(Model model, HttpSession session) {
		...
		List<SimpleMember> memberList = null;

		try {
			memberList = followService.followingPageList(memberId);
	...
	@ResponseBody
	public String followingSubmit(@RequestParam List<Long> checkedValues, Model model, HttpSession session) {
		Long memberId = null;
		SessionInfo sessionInfo = (SessionInfo) session.getAttribute("sessionInfo");
		...
		try {
			for (Long targetId : checkedValues) {
				followService.insertFollow(memberId, targetId);
			}
			return "true";
		} catch (Exception e) {
			e.printStackTrace();
